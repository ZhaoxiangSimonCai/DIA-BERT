...
if __name__ == "__main__":
    ...
    usage_device_list, min_free_memory = get_usage_device(
        common_config_data["gpu"]["max_member_use_rate"]
    )
    ...
    parser.add_option(
        "--rawdata_file_dir_path",
        type="string",
        default="",
        help="Comma-separated mzML paths, a directory, or glob (e.g., /data/run/*.mzML)",
    )

    parser.add_option(
        "--lib", type="string", default="", help="The absolute path to the library file"
    )
    ...
    parser.add_option(
        ...
        type="int",
        default="100",
        help="The search scope for the precursor identification",
    )
    ...
    parser.add_option("--device", type="string", default="cuda", help="Device: [cuda]")

    parser.add_option("--n_thread", type="int", default="5", help="Num of thread")

    parser.add_option(
        "--fitting_rt_num", type="int", default="50000", help="fitting_rt_num"
    )

    parser.add_option(
        "--fitting_rt_batch_size",
        type="int",
        default="100",
    ...
    parser.add_option(
        "--finetune_score_limit",
        type="float",
        default="0.2",
        help="finetune score limit",
    )
    parser.add_option(
        "--train_epochs", type="int", default="10", help="finetune train epochs"
    )

    parser.add_option(
        "--open_lib_decoy", type="int", default="1", help="open lib decoy process"
    )
    parser.add_option(
        "--open_identify", type="int", default="1", help="open identify process"
    )
    parser.add_option(
        "--open_cross_quantification",
        type="int",
        default="0",
        help="Cross-run quantification",
    )
    ...
    parser.add_option(
        "--train_pkl_size",
        type="int",
        default=pkl_size_util.calculate_pkl_size(min_free_memory),
    ...
    parser.add_option(
        ...
        type="int",
        default=0,
        help="Skip DIA-BERT embedding extraction for ProCanFM (0=extract embeddings, 1=skip). Default: 0",
    )
    parser.add_option(
        "--embedding_layer",
        type="string",
        default="penultimate",
        help="Embedding layer for ProCanFM extraction: [penultimate (256-dim, default), final (64-dim)]",
    )
    ...
    if "--help" in args:
        ...
        # ProCanFM embedding extraction (enabled by default)
        input_param.open_embedding_extraction = not bool(
            options.skip_embedding_extraction
        )
        input_param.embedding_layer = options.embedding_layer
